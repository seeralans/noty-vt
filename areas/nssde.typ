...
dynamics of $lambda$, whilst afixing $x$.
...
b^pm_j$ are the Itô corrections on either side. We see that if we wish to
...
The reduced system has sufficently smooth coefficients as wells as inheriting
...
reduces to Itô's lemma. The local time $L^x_t(z)$ can equivalently be defined as
...
we require for Meyer-Itô. For the latter, also need the dynamics of scalar
...
analgously when $z_t$ reaches $pm epsilon$, the switching variable saturates and
the local time increments prevent escape from $[-1, 1]$. The dynamics of the
full system are then represented by the coupled SDE
...
#proof[
By setting $f(x) = ||x||^2 = sum_i (x^((i)))^2$ and applying Itô's lemma to @eq-ito-sde-simp,
  $
...
Having establishsed the scaling of the local time terms, we now estbalish the
fact that no time s time rescaling can balance all contributions to the
$lambda$-dynamics in the follwoing lemma.
...
asymptotic expansion. The objects of interest are not the typical paths obtained
by minimising the Freidlin-Wentzell action functional, but the Gaussian
fluctuations around the most probable path at order $cal(O)(sqrt(epsilon))$.
These phenomena are intrinsically $epsilon$-dependent.
