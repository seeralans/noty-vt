...
+ \mmat{D}(t)
\end{equation}
where $\mmat{A} = \mmat{R} \tilde{J} \mmat{R}^{-1}$ and $\mmat{D} = \mmat{R}\, \tilde{b} \tilde{b}^{\trans}\, \mmat{R}^{\trans}$. If all the eigenvalues of the Jacobian have positive real part, then Equation [[eq-rotation-to-z]] gives a time dependent Ornstein-Uhlenbeck. Notice that we have
