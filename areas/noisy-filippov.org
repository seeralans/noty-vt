...
\end{equation}
We will proceed with Ito calculus on [[eq-sde-al-to-ito]] and later affix the choice
of $\alpha$ for the original problem. 

Recall that away from the discontinuity surface, $\lambda_t \in \{1, -1\}$, and sort

Our goal is to close the SDE given in
[[eq-sde-al-to-ito]] (and in turn [[eq-sde-al]]) in   


In the deterministic scenario, one simply 
...
\right. ,
\end{equation}
are convex functions. We now want to consider the the dynamics of $\lambda_t = \Lambda_{\epsilon}(x_t)$, but obviously since $\Lambda_{\epsilon}(u)$ only has a second derivative in the distributional sense one cannot simply employ Ito's lemma and instead we must use Melyer-Ito. We define the random variable $z_t = \sigma(x_t)$ where $\sigma \in C^2(\mathbb{R}^d)$. Then by Ito's lemma we have
#+name: eq-z-sde
\begin{equation}
\rmd z_t = \ls \mpdiff{\sigma}{x}^{\trans} a + \frac{\epsilon}{2}\mathrm{Tr}(b^{\trans} \mpdiff{\sigma}{2} b) \rs\rmd t
+ \sqrt{\epsilon} \mpdiff{\sigma}{x}^{\trans}  c \rmd W_t.
\end{equation}
For the function $\Lambda_{\epsilon}$ that can be written as the difference of two convex functions, Ito-meyer states that
...
\end{equation}
where $\Lambda^{'-}_{\epsilon}(z_s)$ is the left derivative, $\mu_{\Lambda^{''}_{\epsilon}}(z)$ is the measure of the second derivative and 
#+name: eq-local-time-def
\begin{equation}
L_t(z) \idef \lim_{\delta \to 0}\frac{1}{4\delta} \mu{\lb |z_s - z| < \delta \giv  s \in [0, t] \rb}
\end{equation}
is the so-called local time of $z_t$ at the point $z$. For our case the left derivative of $\Lambda_{\epsilon}(u)$ given by
...
\Lambda_{\epsilon}(z_0) &+ 
\frac{1}{\epsilon}
\int_{0}^{t} \ls \mpdiff{\sigma}{x}^{\trans} a + \frac{\epsilon}{2}\mathrm{Tr}(b^{\trans} \mpdiff{\sigma}{2} b) \rs \rmd s + \frac{1}{\sqrt{\epsilon}}
\int_{0}^{t} \mpdiff{\sigma}{x}^{\trans}  c \rmd W_s \\
...
\rmd \lambda_t &=
\frac{1}{\epsilon}
\mathbb{1}_{(-1, 1)}(\lambda_t)
\lc \frac{1}{\epsilon} \ls \mpdiff{\sigma}{x}^{\trans} a + \frac{\epsilon}{2}\mathrm{Tr}(b^{\trans} \mpdiff{\sigma}{2} b) \rs \rmd t
 + \frac{1}{\sqrt{\epsilon}} \mpdiff{\sigma}{x}^{\trans}  c \rmd W_t \rc \\
&+\frac{1}{\epsilon}
\mathbb{1}_{\{-1\}}(\lambda_t) \rmd L_{t}^{-\epsilon} - \mathbb{1}_{\{1\}}(\lambda_t) \rmd L_{t}^{\epsilon} 
\end{aligned}
\end{equation}
which describes the stochastic evolution of the switching variable.  Let us now consider a smooth function $f \in C^2$, and consider
#+name: eq-mean-f
\begin{align}
&\mathbb{E}[f(\lambda_t) - f(\lambda_0)] =
\mathbb{E}{\ls \int_0^t f'(\lambda_s) \rmd \lambda_s \rs}
+ \mathbb{E}{\ls \int_0^t f'(\lambda_s) \rmd (\lambda, \lambda)_s \rs} \\
 &= \lc \int^{t}_{0}\mathbb{1}_{(-1, 1)}(\lambda_t) \frac{1}{\epsilon}\ls \mpdiff{\sigma}{x}^{\trans} a + \frac{\epsilon}{2}\mathrm{Tr}(b^{\trans} \mpdiff{\sigma}{2} b) 
 + \frac{1}{2}\mpdiff{\sigma}{x}^{\trans}  cc^{\trans} \mpdiff{\sigma}{x} \rs \rmd t \rc +
\mathbb{E}{\ls \int_0^t f'(\lambda_s) \rmd \lambda_s \rs} \\
&+ \frac{1}{\epsilon}\mathbb{E}{\ls \int_0^t f^{\prime \prime}(\lambda_s) 
\mpdiff{\sigma}{x}^{\trans}  c c^{\trans} \mpdiff{\sigma}{x}
\rmd s \rs}
\end{align}

Thus we the generator of $\lambda_t$  
#+name: eq-bws-lam-gen
\begin{equation}
(Af)(\lambda) = \frac{1}{\epsilon}
\ls \mpdiff{\sigma}{x}^{\trans} a + \frac{\epsilon}{2}\mathrm{Tr}(b^{\trans} \mpdiff{\sigma}{2} b) \rs
\mpdiff{f}{\lambda}(\lambda) + \frac{1}{\epsilon} 
\ls \mpdiff{\sigma}{x}^{\trans}
c c^{\trans} \mpdiff{\sigma}{x} \rs \mpdiff[2]{f}{\lambda}(\lambda)
\end{equation}
where
#+name: eq-back-gen-domain
\begin{equation}
\mathcal{D}(A) = {\lc f \in C^2 \giv \mpdiff{f}{\lambda}(-1)  = \mpdiff{f}{\lambda}(1)  = 0 \rc}.
\end{equation}
From which we can obtain the forward generator
#+name: eq-fwd-lam-gen
\begin{equation}
\begin{aligned}
(A^{\dagger}P)(\lambda, t \giv x_t) &=
-\frac{1}{\epsilon}
\mpdiff{\lc P(\lambda, t, \giv x_t)\ls \mpdiff{\sigma}{x}^{\trans} a(x_t, \lambda ) + \frac{\epsilon}{2} \mathrm{Tr}{\lb b^{\trans} \mpdiff[2]{\sigma}{x}b\rb} \rs  \rc}{\lambda} \\
&+\frac{1}{2\epsilon} \mpdiff[2]{[P(\lambda, t, \giv x_t) \mpdiff{\sigma}{x}^{\trans} b b^{\trans}\mpdiff{\sigma}{x} ]}{\lambda}
\end{aligned}
\end{equation}
.... multiplicative noise is (atm) interpreted using the Ito convention ....

We have then the Fokker-Planck equation for $\lambda$ conditioned on the state $x_t = x$
#+name: eq-lam-fokker-plank-slow-t
\begin{equation}
\begin{aligned}
\mpdiff{P(\lambda, t | x)}{t} = 
-\frac{1}{\epsilon}
\mpdiff{\lc P(\lambda, t \giv x)\ls \mpdiff{\sigma}{x}^{\trans} a(x, \lambda ) + \frac{\epsilon}{2} \mathrm{Tr}{\lb b^{\trans} \mpdiff[2]{\sigma}{x}b\rb} \rs  \rc}{\lambda} \\
+\frac{1}{2\epsilon} \mpdiff[2]{\lc P(\lambda, t \giv x) \mpdiff{\sigma}{x}^{\trans} b b^{\trans}\mpdiff{\sigma}{x} \big. \rc}{\lambda},
\end{aligned}
\end{equation}
with the boundary condition
#+name: eq-lam-fokker-plank-bc
\begin{equation}
 \frac{1}{\epsilon} {
\mpdiff{\lc P(\lambda, t \giv x)\ls \mpdiff{\sigma}{x}^{\trans} a(x, \lambda ) + \frac{\epsilon}{2} \mathrm{Tr}{\lb b^{\trans} \mpdiff[2]{\sigma}{x}b\rb} \rs  
+\frac{1}{2} \mpdiff{\lc P(\lambda, t \giv x) \mpdiff{\sigma}{x}^{\trans} b b^{\trans}\mpdiff{\sigma}{x} \big. \rc}{\lambda} \rc}{\lambda}\bigg{|}_{\lambda \pm 1}  = 0
}.
\end{equation}
Switching to fast time $t = \epsilon \tau$ before taking the limit we obtain  
#+name: eq-lam-fokker-plank-fast-t
\begin{equation}
\begin{aligned}
\mpdiff{P(\lambda, \tau | x)}{\tau} = 
- \mpdiff{\lc  P(\lambda, \tau \giv x) \Bigg. \ls \mpdiff{\sigma}{x}^{\trans} a(x, \lambda )  \rs  \rc}{\lambda} 
+\frac{1}{2} \mpdiff[2]{\lc P(\lambda, \tau \giv x) \mpdiff{\sigma}{x}^{\trans} b b^{\trans}\mpdiff{\sigma}{x} \big. \rc}{\lambda},
\end{aligned}
\end{equation}
with the boundary condition
...
\begin{equation}
{
\mpdiff{\lc P(\lambda, \tau \giv x)\ls \mpdiff{\sigma}{x}^{\trans} a(x, \lambda ) \rs  
+\frac{1}{2} \mpdiff{\lc P(\lambda, \tau \giv x) \mpdiff{\sigma}{x}^{\trans} b b^{\trans}\mpdiff{\sigma}{x} \big. \rc}{\lambda} \rc}{\lambda}\bigg{|}_{\lambda \pm 1}  = 0
}.
\end{equation}
